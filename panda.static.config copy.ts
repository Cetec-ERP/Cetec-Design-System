...
const dumpEnums = () => {
  ...
  if (process.env.PANDA_DEBUG_ENUMS === '1') {
    ...
    try {
      ...
      const utilEntries = Object.entries(
        (basePresetObj as any)?.utilities ?? {},
      ...
      require('fs').writeFileSync(
        keysOut,
        JSON.stringify({ utilities: utilEntries }, null, 2),
        'utf8',
      );
      require('fs').writeFileSync(
        out,
    ...
    css: [
      ...
      // Flex & Grid
      {
        properties: {
          ...
          flexShrink: ['*'],
          flexBasis: ['*'],
          flexDirection: U_FLEX_DIRECTION.values,
          flexWrap: U_FLEX_WRAP.values,
          order: ['*'],
        },
        responsive: true,
      },
      {
        properties: {
          alignItems: U_ALIGN_ITEMS.values,
          justifyItems: U_JUSTIFY_ITEMS.values,
          alignContent: U_ALIGN_CONTENT.values,
          justifyContent: U_JUSTIFY_CONTENT.values,
          placeItems: U_PLACE_ITEMS.values,
          placeContent: U_PLACE_CONTENT.values,
        },
        responsive: true,
      },
      {
        properties: {
          alignSelf: U_ALIGN_SELF.values,
          justifySelf: U_JUSTIFY_SELF.values,
          placeSelf: U_PLACE_SELF.values,
        },
        responsive: true,
      },
      {
      ...
      // Typography
      {
        properties: {
          ...
          lineHeight: ['*'],
          letterSpacing: ['*'],
          textAlign: U_TEXT_ALIGN.values,
          textTransform: U_TEXT_TRANSFORM.values,
          textDecoration: U_TEXT_DECORATION.values,
          whiteSpace: ['*'],
          wordBreak: ['*'],
        },
        responsive: true,
      ...
      {
        properties: {
          overflow: U_OVERFLOW.values,
          objectFit: U_OBJECT_FIT.values,
          objectPosition: U_OBJECT_POSITION.values,
        },
        responsive: true,
      },
