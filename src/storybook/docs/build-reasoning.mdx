...
2. Keep the Panda preset in its own `./preset` entrypoint so the component runtime does not depend on Panda build tooling.
...
- The preset is available from `cetec-design-system/preset` rather than the component runtime entrypoint.
...
- The preset entrypoint keeps its Panda types isolated from component-only consumers.


### Distribution Output

The expected `dist` shape includes separate component and preset bundles. Component consumers use the root entrypoint, while Panda configuration imports the build-time preset from `cetec-design-system/preset`.
